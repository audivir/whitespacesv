...
# constant lookup of variables will hit performance
# but way easier to read
# maybe enclose in "if TYPE_CHECKING" block
...
def is_ord_whitespace(c: int) -> bool:
    ...
    return (
        c == 0x09  # TAB
        or (0x0B <= c <= 0x0D)  # LINE_TAB, FORM_FEED, CARRIAGE_RETURN
        or c == 0x20  # SPACE
        or c == 0x85  # NEXT_LINE
        or c == 0xA0  # NO_BREAK_SPACE
        or c == 0x1680  # OGHAM_SPACE_MARK
        or (0x2000 <= c <= 0x200A)  # EN_QUAD to HAIR_SPACE
        or c == 0x2028  # LINE_SEPARATOR
        or c == 0x2029  # PARAGRAPH_SEPARATOR
        or c == 0x202F  # NARROW_NO_BREAK_SPACE
        or c == 0x205F  # MEDIUM_MATHEMATICAL_SPACE
        or c == 0x3000  # IDEOGRAPHIC_SPACE
    )
...
def contains_string_special_chars(value: str) -> bool:
    ...
    return any(
        c == 0x0A  # NEW_LINE
        or is_ord_whitespace(c)
        or c == 0x22  # DOUBLE_QUOTE
        or c == 0x23  # HASH
        for c in ords
    )
...
class WsvCharIterator(TxtCharIterator):
    ...
    def jump(self, eol: bool = False) -> int:
        ...
        while True:
            ...
            if c == 0x0A:  # NEW_LINE # pylint: disable=consider-using-assignment-expr
                break
    ...
    def get_exc(self, message: str) -> WsvParserException:
        """Returns a WsvParserException with the current line and position"""
        line_ix, line_position = self.get_line_info()
        return WsvParserException(self.ix, line_ix, line_position, message)
    ...
    def read_value(self) -> str:
        ...
        while True:
            ...
            c = self._chars[self.ix]
            if is_ord_whitespace(c) or c == 0x0A or c == 0x23:  # NEW_LINE  # HASH
                break
...
def reinfer_types(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = pd.read_csv(StringIO(tmp.getvalue()))
    return df
